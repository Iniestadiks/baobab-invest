...
router.get('/investor', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/entrepreneur', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/mentor', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const projects = await prisma.project.findMany({
...
router.get('/admin', authenticate, requireAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const [revenues, projects, users, investments] = await Promise.all([
      ...
      prisma.project.findMany({ select: { status: true, raisedAmount: true, sector: true, createdAt: true } }),
