...
router.post('/', authenticate, requireRole(['ENTREPRENEUR']), async (req, res) => {
    try {
...
router.post('/:id/request-early-close', authenticate, async (req, res) => {
    try {
        const { note } = req.body;
        const project = await prisma.project.findUnique({ where: { id: req.params.id } });
...
router.post('/:id/request-extension', authenticate, async (req, res) => {
    try {
...
router.post('/:id/approve-early-close', authenticate, requireAdmin, async (req, res) => {
    try {
        const project = await prisma.project.findUnique({
            where: { id: req.params.id },
...
router.post('/:id/approve-extension', authenticate, requireAdmin, async (req, res) => {
    try {
        const project = await prisma.project.findUnique({ where: { id: req.params.id } });
        if (!project) {
            res.status(404).json({ success: false, message: 'Projet introuvable' });
...
router.get('/:id/investors', authenticate, async (req, res) => {
    try {
        const project = await prisma.project.findUnique({ where: { id: req.params.id } });
        if (!project) {
            res.status(404).json({ success: false, message: 'Projet introuvable' });
...
router.get('/stats/entrepreneur', authenticate, requireRole(['ENTREPRENEUR']), async (req, res) => {
    try {
        const projects = await prisma.project.findMany({
            where: { entrepreneurId: req.userId },
            include: {
...
router.post('/:id/mentor/accept', authenticate, requireRole(['MENTOR']), async (req, res) => {
    try {
        const project = await prisma.project.findUnique({ where: { id: req.params.id } });
        if (!project) {
            res.status(404).json({ success: false, message: 'Projet introuvable' });
...
router.post('/:id/mentor/decline', authenticate, requireRole(['MENTOR']), async (req, res) => {
    try {
        const project = await prisma.project.findUnique({ where: { id: req.params.id } });
...
router.get('/mentor/my-projects', authenticate, requireRole(['MENTOR']), async (req, res) => {
    try {
        const projects = await prisma.project.findMany({
