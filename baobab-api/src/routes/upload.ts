...
// ─── UPLOAD PITCH VIDEO ──────────────────────────────────────────────────────
router.post('/pitch-video', authenticate, upload.single('video'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.file) {
      errorResponse(res, 'Aucun fichier vidéo reçu', 400); return
    }
...
router.delete('/pitch-video/:publicId', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    // Vérifier que c'est un admin
    const user = await prisma.user.findUnique({ where: { id: req.userId! } })
    if (user?.role !== 'ADMIN') {
      errorResponse(res, 'Accès non autorisé', 403); return
    }
