...
router.post('/project/:projectId', authenticate, requireRole(['ENTREPRENEUR']), async (req, res) => {
    try {
...
router.get('/project/:projectId/budget', authenticate, async (req, res) => {
    try {
        const project = await prisma.project.findUnique({
...
router.post('/:id/request', authenticate, requireRole(['ENTREPRENEUR']), async (req, res) => {
    try {
...
router.post('/project/:projectId/reimburse', authenticate, requireAdmin, async (req, res) => {
    try {
        const projectId = req.params.projectId;
        const project = await prisma.project.findUnique({
            where: { id: projectId },
