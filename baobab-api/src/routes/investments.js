...
import { addReputationPoints, checkAndAwardBadges, REPUTATION_POINTS } from '../services/reputationService';
...
router.post('/:projectId', authenticate, async (req, res) => {
    try {
        ...
        if (invCount === 1)
            await addReputationPoints(req.userId, 'FIRST_INVESTMENT', REPUTATION_POINTS.FIRST_INVESTMENT, 'Premier investissement effectué', projectId);
        else
            await addReputationPoints(req.userId, 'INVESTMENT_MADE', REPUTATION_POINTS.INVESTMENT_MADE, 'Nouvel investissement effectué', projectId);
        if (amount >= 1000000)
            await addReputationPoints(req.userId, 'INVESTMENT_1M', REPUTATION_POINTS.INVESTMENT_1M, 'Investissement > 1 000 000 FCFA', projectId);
        else if (amount >= 500000)
            await addReputationPoints(req.userId, 'INVESTMENT_500K', REPUTATION_POINTS.INVESTMENT_500K, 'Investissement > 500 000 FCFA', projectId);
        else if (amount >= 100000)
            await addReputationPoints(req.userId, 'INVESTMENT_100K', REPUTATION_POINTS.INVESTMENT_100K, 'Investissement > 100 000 FCFA', projectId);
        else if (amount >= 50000)
            await addReputationPoints(req.userId, 'INVESTMENT_50K', REPUTATION_POINTS.INVESTMENT_50K, 'Investissement > 50 000 FCFA', projectId);
...
router.post('/reimburse-project/:projectId', authenticate, async (req, res) => {
    try {
...
// Plans d'épargne
router.get('/savings-plans', authenticate, async (req, res) => {
    try {
        const wallet = await prisma.wallet.findUnique({ where: { userId: req.userId } });
...
router.post('/savings-plan', authenticate, async (req, res) => {
    try {
...
router.post('/savings-config', authenticate, async (req, res) => {
    try {
        const { amount, day } = req.body;
        if (!amount || amount < 1000) {
...
// Export CSV
router.get('/exports/admin', authenticate, async (req, res) => {
    try {
        const investments = await prisma.investment.findMany({
