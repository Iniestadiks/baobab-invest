...
router.patch('/admin/verify/:userId', authenticate, requireAdmin, async (req, res) => {
    try {
...
router.get('/admin/dossier/:userId', authenticate, requireAdmin, async (req, res) => {
    try {
...
router.post('/check-expiry', authenticate, requireAdmin, async (req, res) => {
    try {
        const now = new Date();
