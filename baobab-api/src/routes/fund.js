...
import { updateBuilderGamification, updateTopDonor } from '../services/builderGamification';
const router = Router();
...
router.post('/admin/confirm/:id', authenticate, requireAdmin, async (req, res) => {
    try {
        const contribution = await prisma.fundContribution.findUnique({ where: { id: req.params.id } });
        if (!contribution) {
...
router.post('/admin/allocate', authenticate, requireAdmin, async (req, res) => {
    try {
...
router.get('/builder/impact', authenticate, async (req, res) => {
    try {
...
router.patch('/admin/builder/:userId/verify', authenticate, requireAdmin, async (req, res) => {
    try {
