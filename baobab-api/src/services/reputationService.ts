...
// ===== NIVEAUX =====
export function getLevel(points: number): { level: number; label: string; icon: string; nextLevelPoints: number } {
  if (points >= 1000) return { level: 5, label: "Grand Baobab", icon: "🏆", nextLevelPoints: 9999 }
  if (points >= 600)  return { level: 4, label: "Baobab",       icon: "🌲", nextLevelPoints: 1000 }
  if (points >= 300)  return { level: 3, label: "Arbre",        icon: "🌳", nextLevelPoints: 600 }
  if (points >= 100)  return { level: 2, label: "Pousse",       icon: "🌿", nextLevelPoints: 300 }
...
// ===== ATTRIBUER UN BADGE =====
export async function awardBadge(userId: string, badgeKey: string, projectId?: string) {
  try {
...
export async function checkAndAwardBadges(userId: string, role: string) {
  try {
    ...
    if (role === "INVESTOR") {
      ...
      if (investCount >= 1 && !existingBadges.includes("FIRST_INVESTMENT")) await awardBadge(userId, "FIRST_INVESTMENT")
      if (investCount >= 5 && !existingBadges.includes("INVESTOR_ACTIVE")) await awardBadge(userId, "INVESTOR_ACTIVE")
      if (totalInvested >= 1000000 && !existingBadges.includes("INVESTOR_DIAMOND")) await awardBadge(userId, "INVESTOR_DIAMOND")
      if (sectors.length >= 5 && !existingBadges.includes("INVESTOR_DIVERSIFIED")) await awardBadge(userId, "INVESTOR_DIVERSIFIED")
      if (user.country && user.country !== "SN" && !existingBadges.includes("INVESTOR_DIASPORA")) await awardBadge(userId, "INVESTOR_DIASPORA")
    }
    ...
    if (role === "ENTREPRENEUR") {
      ...
      if (successProjects >= 2 && !existingBadges.includes("ENTREPRENEUR_REPEAT")) await awardBadge(userId, "ENTREPRENEUR_REPEAT")
    }
  } catch (e) {
