...
async function sendVerificationEmail(email, firstName, code) {
    try {
      ...
      `,
        ...
        console.error('[EMAIL] Erreur envoi code vérification:', e.message);
    }
}
// ═══════════════════════════════════════════════════════
// RATE LIMITERS
...
function isCurrentlyBanned(user) {
    ...
    if (user.rehabilitationAt && new Date() > user.rehabilitationAt)
        return false;
    return true;
}
/** Déblocage automatique si la date de réhabilitation est passée. */
async function autoUnbanIfExpired(userId, rehabilitationAt) {
...
router.post('/login', loginLimiter, async (req, res) => {
    try {
        ...
        const valid = await bcryptjs_1.default.compare(password, freshUser.password);
        if (!valid) {
            res.status(401).json({ success: false, message: 'Identifiants invalides' });
            return;
        }
        const accessToken = (0, jwt_1.generateAccessToken)(freshUser.id, freshUser.role);
        const refreshToken = (0, jwt_1.generateRefreshToken)(freshUser.id);
...
router.post('/register', registerLimiter, async (req, res) => {
    try {
        ...
        const [existingEmail, existingPhone] = await Promise.all([
            database_1.default.user.findUnique({ where: { email: normalizedEmail } }),
            database_1.default.user.findUnique({ where: { phone: phone.trim() } }),
        ]);
        if (existingEmail) {
...
router.post('/verify-email', async (req, res) => {
    try {
        ...
        (0, helpers_1.successResponse)(res, { verified: true }, 'Email vérifié avec succès ✅');
    }
    catch (e) {
...
router.post('/resend-verify', async (req, res) => {
    try {
        ...
        (0, helpers_1.successResponse)(res, {}, 'Nouveau code envoyé');
    }
    catch (e) {
...
router.get('/google/callback', async (req, res) => {
    const frontendUrl = process.env.FRONTEND_URL || config_1.config.app.frontendUrl;
    try {
        ...
        if (user) {
            await autoUnbanIfExpired(user.id, user.rehabilitationAt);
            user = await database_1.default.user.findUnique({ where: { id: user.id } });
        }
        ...
        if (!user) {
            ...
            user = await database_1.default.user.create({
                data: {
                    ...
                    password: await bcryptjs_1.default.hash(Math.random().toString(36) + Date.now(), 10),
                    role: 'INVESTOR',
                    referralCode,
        ...
        res.redirect(`${process.env.FRONTEND_URL || config_1.config.app.frontendUrl}/auth/login?error=google_failed`);
    }
});
// ═══════════════════════════════════════════════════════
// REFRESH TOKEN
...
router.post('/refresh', async (req, res) => {
    try {
        ...
        const newAccessToken = (0, jwt_1.generateAccessToken)(freshUser.id, freshUser.role);
        const newRefreshToken = (0, jwt_1.generateRefreshToken)(freshUser.id);
        (0, helpers_1.successResponse)(res, { accessToken: newAccessToken, refreshToken: newRefreshToken });
    }
    catch (e) {
...
router.patch('/profile', auth_1.authenticate, async (req, res) => {
    try {
        const { firstName, lastName, phone, city, country, bio, region, countryCode, stateCode, indicatif } = req.body;
...
router.patch('/builder/profile', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const profile = await database_1.default.builderProfile.upsert({
            ...
            create: { userId: req.userId, companyName: companyName, sector: sector, description: description, website: website, country: country, isPublic: Boolean(isPublic) },
            update: { companyName: companyName, sector: sector, description: description, website: website, country: country, isPublic: Boolean(isPublic), updatedAt: new Date() },
        });
...
// ═══════════════════════════════════════════════════════
async function logAdminAction(adminId, action, targetUserId, reason, ip, ua) {
    try {
        await database_1.default.adminAuditLog.create({
            data: {
                adminId,
                action,
                targetUserId,
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        if (!email || !code || !newPassword) {
            res.status(400).json({ success: false, message: 'Email, code et nouveau mot de passe requis' });
            return;
        }
        if (newPassword.length < 8) {
            res.status(400).json({ success: false, message: 'Le mot de passe doit contenir au moins 8 caracteres' });
            return;
        }
        const user = await database_1.default.user.findUnique({ where: { email: email.toLowerCase().trim() } });
        if (!user || !user.emailVerifyCode || !user.emailVerifyExpiry) {
            res.status(400).json({ success: false, message: 'Code invalide ou expire' });
            return;
        }
        if (new Date() > user.emailVerifyExpiry) {
            res.status(400).json({ success: false, message: 'Code expire — recommencez' });
            return;
        }
        if (user.emailVerifyCode !== code.trim()) {
            res.status(400).json({ success: false, message: 'Code incorrect' });
        ...
        (0, helpers_1.successResponse)(res, {}, 'Mot de passe reinitialise avec succes');
    }
    catch (e) {
