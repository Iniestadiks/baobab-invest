...
async function awardFundBadges(userId) {
    ...
    if (amount >= 500)
        badges.push('SEMEUR');
    if (amount >= 5000)
        badges.push('JARDINIER');
    if (amount >= 25000)
        badges.push('BAOBAB');
    if (amount >= 100000)
        badges.push('GRAND_BATISSEUR');
...
async function finalizeContribution(contributionId) {
    ...
    if (contribution.userId) {
        ...
        await (0, builderGamification_1.updateBuilderGamification)(contribution.userId, { type: 'DON_FONDS', amount: contribution.amount });
...
router.get('/stats', async (req, res) => {
    try {
      ...
      `
        ...
        const enrichedTop = await Promise.all(topContributors.map(async (c) => {
            if (!c.userId)
                return null;
...
router.get('/contributions', async (req, res) => {
    try {
        ...
        successResponse(res, { contributions: enriched, total, page: parseInt(page), pages: Math.ceil(total / parseInt(limit)) });
    }
    catch (e) {
        errorResponse(res);
    }
...
router.post('/contribute', async (req, res) => {
    try {
        ...
        if (!amount || amount < 500) {
            errorResponse(res, 'Montant minimum : 500 FCFA', 400);
            return;
        }
...
router.post('/webhook/paydunya', async (req, res) => {
    try {
        ...
        const customData = confirmation.raw?.custom_data || {};
...
router.post('/admin/confirm/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const contribution = await prisma.fundContribution.findUnique({ where: { id: req.params.id } });
        if (!contribution) {
            errorResponse(res, 'Contribution introuvable', 404);
            return;
        }
...
router.post('/admin/allocate', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { projectId, amount, note, justification } = req.body;
        if (!projectId || !amount) {
            errorResponse(res, 'Projet et montant requis', 400);
            return;
        }
        if (!justification) {
            errorResponse(res, 'Justification obligatoire pour toute allocation du Fonds Solidaire', 400);
            return;
        }
        ...
        if (amount > available) {
            errorResponse(res, `Fonds insuffisant. Disponible: ${Math.round(available).toLocaleString()} FCFA`, 400);
            return;
        }
        const project = await prisma.project.findUnique({ where: { id: projectId }, include: { entrepreneur: true } });
        if (!project) {
            errorResponse(res, 'Projet introuvable', 404);
            return;
        }
        ...
        for (const c of contributors) {
            if (c.userId) {
                await (0, builderGamification_1.updateBuilderGamification)(c.userId, { type: 'FONDS_UTILISE' });
            }
        }
        await (0, builderGamification_1.updateTopDonor)();
...
router.get('/admin/contributions', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        successResponse(res, { contributions, total, stats: stats._sum, pages: Math.ceil(total / parseInt(limit)) });
    }
    catch (e) {
        errorResponse(res);
    }
...
router.post('/admin/campaigns', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        if (!title || !goalAmount || !startDate || !endDate) {
            errorResponse(res, 'Champs manquants', 400);
            return;
        }
...
router.get('/admin/export', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
...
router.get('/my-contributions', auth_1.authenticate, async (req, res) => {
    try {
...
router.get('/builder/impact', auth_1.authenticate, async (req, res) => {
    try {
...
router.get('/builders/public', async (req, res) => {
    try {
...
router.patch('/admin/builder/:userId/verify', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
