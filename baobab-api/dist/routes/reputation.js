...
router.get("/me", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const levelInfo = (0, reputationService_1.getLevel)(user.reputationPoints || 0);
...
router.get("/leaderboard", async (req, res) => {
    try {
        ...
        const includeExtra = (r) => {
            if (r === "INVESTOR")
                return { investments: { select: { amount: true } } };
            if (r === "ENTREPRENEUR")
                return { projectsOwned: { select: { status: true, raisedAmount: true } } };
            return {};
        };
        const mapResult = (u) => ({
            ...u,
            totalInvested: u.investments ? u.investments.reduce((s, i) => s + i.amount, 0) : undefined,
        ...
        if (period === "all") {
            ...
            res.json({ success: true, data: users.map(mapResult) });
            return;
        }
...
router.post("/award-badge", auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { userId, badgeKey } = req.body;
        await (0, reputationService_1.awardBadge)(userId, badgeKey);
