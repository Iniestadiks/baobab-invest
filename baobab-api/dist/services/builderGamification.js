...
exports.BADGE_THRESHOLDS = void 0;
exports.calcPoints = calcPoints;
exports.getBadgeFromPoints = getBadgeFromPoints;
...
function getBadgeFromPoints(points) {
    let badge = 'SEMEUR';
    for (const t of exports.BADGE_THRESHOLDS) {
        if (points >= t.points)
            badge = t.badge;
    }
    return badge;
}
async function updateBuilderGamification(userId, action) {
    try {
        ...
        if (newStreak >= 3 && !specialBadges.includes('FIDELE'))
            specialBadges.push('FIDELE');
        const totalContributors = await prisma.fundContribution.count({ where: { status: 'COMPLETED' } });
        if (totalContributors <= 1 && !specialBadges.includes('FONDATEUR'))
            specialBadges.push('FONDATEUR');
        ...
        console.error('[GAMIFICATION] Erreur:', e);
    }
}
async function updateTopDonor() {
    try {
        ...
        if (!badges.includes('ROI_FONDS'))
            badges.push('ROI_FONDS');
        await prisma.builderProfile.update({
            where: { id: top.id },
        ...
        console.error('[GAMIFICATION] updateTopDonor:', e);
    }
}
async function decrementInactiveBuilders() {
    try {
        ...
        for (const p of profiles) {
            ...
            if (decrement > 0) {
                ...
                const currentThreshold = exports.BADGE_THRESHOLDS.find(t => t.badge === currentBadge)?.points || 0;
                const prevThreshold = [...exports.BADGE_THRESHOLDS].reverse().find(t => t.points < currentThreshold)?.points || 0;
        ...
        console.error('[GAMIFICATION] decrementInactive:', e);
    }
}
//# sourceMappingURL=builderGamification.js.map