...
exports.checkAndPayReferralBonus = checkAndPayReferralBonus;
async function getReferralConfig(tx) {
...
async function hasInvested(userId, tx) {
    const count = await tx.investment.count({ where: { userId } });
    return count > 0;
}
async function payReferralBonus(parrainId, filleulId, tx) {
    const cfg = await getReferralConfig(tx);
    if (!cfg.programActive)
        return false;
...
async function checkAndPayReferralBonus(userId, tx) {
    try {
