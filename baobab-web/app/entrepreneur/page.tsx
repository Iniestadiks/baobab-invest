                        ...
                        {(p.status === 'FUNDED' || p.status === 'IN_PROGRESS') && (
                          <div className="mt-2 grid grid-cols-3 gap-1 text-xs">
                            {[
                              ...
                              { n: 2, label: "25%", amount: Math.round((p.netAmount||0)*0.25), done: (p.currentPalier||0) >= 2 },
                              { n: 3, label: "35%", amount: Math.round((p.netAmount||0)*0.35), done: (p.currentPalier||0) >= 3 },
                            ].map(pl => (
                ...
                <div key={pid} className={`bg-white rounded-2xl border-2 shadow-sm p-5 ${isLate ? 'border-red-300' : 'border-gray-100'}`}>
                  <div className="flex items-center justify-between mb-3 cursor-pointer"
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{project?.sector} · Remboursement {sc.paidMonths}/{sc.totalMonths} mois</p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {sc.payments?.map((pay: any) => {
                        ...
                        return (
                          <div key={pay.id}
                            ...
                            <div className="font-semibold">{pay.amount.toLocaleString("fr-FR")} FCFA</div>
                            <div className="text-gray-400 text-xs">{new Date(pay.dueDate).toLocaleDateString("fr-FR", {day:"numeric", month:"short", year:"numeric"})}</div>
                          </div>
                        );
                      })}
                    </div>
