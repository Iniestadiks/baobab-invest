  ...
  return (
    <>
      ...
      <div style={{minHeight:"100vh",background:"var(--cream)",paddingTop:72}}>
        <div style={{background:"linear-gradient(180deg,rgba(124,58,237,0.05) 0%,transparent 100%)",borderBottom:"1px solid var(--line)",padding:"64px 32px 48px"}}>
          <div style={{maxWidth:1280,margin:"0 auto"}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(124,58,237,0.06)",border:"1px solid rgba(124,58,237,0.16)",borderRadius:100,padding:"6px 16px",fontSize:11.5,fontWeight:700,color:"#7C3AED",marginBottom:24}}>🔒 Vie privée</div>
            <h1 style={{fontSize:"clamp(32px,5vw,56px)",fontWeight:900,letterSpacing:"-2px",marginBottom:16,color:"var(--ink)"}}>Politique de<br/><span style={{color:"#7C3AED"}}>Confidentialité</span></h1>
            ...
            <div style={{display:"flex",gap:24,marginTop:20,flexWrap:"wrap"}}>
              {[{icon:"🚫",label:"Données jamais vendues"},{icon:"🔒",label:"Chiffrement SSL/TLS"},{icon:"✅",label:"Droits RGPD respectés"}].map(b=>(
                <div key={b.label} style={{display:"flex",alignItems:"center",gap:8,fontSize:13,color:"var(--ink-soft)",fontWeight:500}}>
          ...
          <div style={{position:"sticky",top:96,background:"#fff",border:"1px solid var(--line)",borderRadius:20,padding:20,boxShadow:"0 1px 3px rgba(11,17,32,0.03)"}}>
            ...
            {SECTIONS.map(s => (
              <a key={s.id} href={"#"+s.id} onClick={() => setActive(s.id)}
                style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",borderRadius:12,marginBottom:4,fontSize:13,fontWeight:active===s.id?700:500,color:active===s.id?"#7C3AED":"var(--ink-soft)",background:active===s.id?"rgba(124,58,237,0.06)":"transparent",transition:"all 0.2s",borderLeft:active===s.id?"2px solid #7C3AED":"2px solid transparent"}}>
            ...
            {SECTIONS.map(s => (
              <div key={s.id} id={s.id} style={{background:"#fff",border:"1px solid var(--line)",borderRadius:24,padding:40,scrollMarginTop:96,boxShadow:"0 1px 3px rgba(11,17,32,0.03), 0 8px 20px rgba(11,17,32,0.02)"}}>
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
                  ...
                  <h2 style={{fontSize:20,fontWeight:800,color:"var(--ink)"}}>{s.title}</h2>
                </div>
                <div style={{fontSize:14.5,color:"var(--ink-soft)",lineHeight:1.85,whiteSpace:"pre-line"}}>{s.content}</div>
              </div>
            ))}
            <div style={{background:"linear-gradient(135deg,rgba(124,58,237,0.06),rgba(22,163,74,0.04))",border:"1px solid rgba(124,58,237,0.14)",borderRadius:24,padding:40,textAlign:"center"}}>
              <div style={{fontSize:32,marginBottom:16}}>🔐</div>
              <h3 style={{fontSize:20,fontWeight:800,marginBottom:10,color:"var(--ink)"}}>Exercer vos droits</h3>
              <p style={{fontSize:14,color:"var(--ink-soft)",marginBottom:24}}>Réponse sous 30 jours maximum.</p>
              <a href="mailto:privacy@korapact.com" style={{display:"inline-block",background:"linear-gradient(135deg,#7C3AED,#6D28D9)",color:"#fff",fontWeight:700,fontSize:14,padding:"13px 28px",borderRadius:14,boxShadow:"0 8px 20px rgba(124,58,237,0.25)"}}>privacy@korapact.com →</a>
            </div>
          </div>
        </div>
